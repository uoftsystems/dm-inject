...
static int lostwrite_map(struct dm_target *ti, struct bio *bio/*,union map_info *map_context*/)
{
	...
	switch (bio_data_dir(bio)) {
		...
		case WRITE:
			...
			// 	unlike shorn writes, lost writes are transient, i.e. if another
			//	write happens to the same logical block again, the block will
			// 	*NOT* be discarded.
				mdt->corrupt = false;
				bio_endio(bio);
...
static int 
lostwrite_ctr(struct dm_target *ti,unsigned int argc,char **argv)
{
	...
	printk("%s():\n",__func__);	
        ...
        if(mdt==NULL)
        {
                ti->error = "dm-lostwrite: Cannot allocate linear context";
                return -ENOMEM;
        }       
        if(sscanf(argv[1], "%llu", &start)!=1)
        {
                ti->error = "dm-lostwrite: Invalid device sector";
                goto bad;
        }
	...
	r = setup_corruption(mdt, ti);
       /* dm_get_table_mode 
        ...
        if (dm_get_device(ti, argv[0], dm_table_get_mode(ti->table), &mdt->dev)) {
                ti->error = "dm-lostwrite: Device lookup failed";
                goto bad;
        }
...
static void lostwrite_dtr(struct dm_target *ti)
{
        struct my_dm_target *mdt = (struct my_dm_target *) ti->private;
	printk("%s():\n",__func__);	
...
static int lostwrite_end_io(struct dm_target *ti, struct bio *bio, blk_status_t *error)
{
...
//static int lostwrite_message(struct dm_target *ti,unsigned int argc, char **argv){
static int lostwrite_message(struct dm_target *ti,unsigned int argc, char **argv, char *unused1, unsigned unused2){
//	printk(KERN_INFO "dm-lostwrite message received: %s", argv[0]);
...
static struct target_type lostwrite = {
        .name = "lostwrite",
        .version = {1,0,0},
        .module = THIS_MODULE,
        .ctr = lostwrite_ctr,
        .dtr = lostwrite_dtr,
        .map = lostwrite_map,
	.end_io = lostwrite_end_io,
	.message = lostwrite_message,
};

/*---------Module Functions -----------------*/

static int init_lostwrite(void)
{
        int result;
        result = dm_register_target(&lostwrite);
        if(result < 0)
                printk(KERN_CRIT "\n Error in registering target \n");
        return 0;
}

static void cleanup_lostwrite(void)
{
        dm_unregister_target(&lostwrite);
}
module_init(init_lostwrite);
module_exit(cleanup_lostwrite);
MODULE_LICENSE("GPL");
